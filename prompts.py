...
JSON_FORMAT_INSTRUCTIONS_ANTHROPIC = """CRITICAL: Your response must be ONLY a valid JSON object.
...
THREAT_INTEL_EXTRACTION_PROMPT = """You are a senior threat intelligence analyst specializing in extracting precise, actionable intelligence from security reports. 
...
Extract information PRECISELY as written in the source. Do not add interpretations."""

# SPL Rule Generation
SPL_GENERATION_PROMPT = """You are a senior detection engineer creating high-fidelity Splunk SPL detection rules.
...
3. Include statistical analysis (stats, rare, outlier detection) where appropriate
...
{{
  "rules": [
    {{
      ...
      "description": "Detects abnormal data exfiltration from Salesforce instances",
      "rule_content": "index=salesforce sourcetype=salesforce:api | stats sum(bytes_out) as total_bytes by user | where total_bytes > 1000000000",
    ...
    {{
      ...
      "rule_content": "index=cloud_logs (action=login OR action=auth) | stats count by src_ip user | where count > 100",
...
SIGMA_GENERATION_PROMPT = """You are a senior detection engineer specializing in Sigma rule development.
...
2. Use appropriate log sources (process_creation, network_connection, file_event, etc.)
...
{{
  "rules": [
    ...
    {{
      ...
      "description": "Identifies abnormal authentication patterns indicative of credential abuse",
